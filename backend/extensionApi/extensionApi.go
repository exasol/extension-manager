...
func readRequiredStringProperty(extensionJs *otto.Object, propertyName string) (*string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invalid value for extension property %v. The value must be a sting. Cause: %v", propertyName, err.Error())
	}
...
func loadExtension(vm *otto.Otto, fileName string) (*otto.Object, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failled to set installedExtension = null. Cause: %v", err.Error())
	}
	file, err := os.Open(fileName)
	if err != nil {
		return nil, fmt.Errorf("failed to open installedExtension file. Cause: %v", err.Error())
	}
	_, err = vm.Run(file)
	if err != nil {
		return nil, fmt.Errorf("failed to load installedExtension. Cause %v", err.Error())
	}
...
func getApiVersion(installedExtensionObject *otto.Object) (*string, error) {
	...
	if !apiVersion.IsString() {
		return nil, fmt.Errorf("invalid installedExtension.apiVersion. The filed should be a string")
	}
